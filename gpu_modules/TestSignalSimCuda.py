...
__device__ float interpolate1d_cl(float x, float * array_x){
    int ndimx = 100;
    float ystep = 0.01;
    float ymin = 0.03;
    float ymax = 1.03;
    ...
    if(x < array_x[0])return ymin + 0.5*ystep;
    else if(x>array_x[ndimx-1])return ymax+0.5*ystep;
    else{
            int i;
            for(i = 0; i < ndimx ; i++){
                if(x > array_x[i]);
            ...
            float x0 = array_x[i-1];
            float x1 = array_x[i];
            float y0 = (i-1) * ystep + ymin + 0.5 * ystep;
            float y1 = i * ystep + ymin + 0.5 * ystep;
           // printf("%d, %f, %f, %f, %f, %f\\n", i, x, x0, x1, y0, y1);
            float temp = y0 + (y1 - y0)/(x1 - x0) * (x - x0);
            return temp;
        }
}

__device__ float legendreSeries_rmean(float xx, float *par ){
    //ATTENTION:normX is energymax, please remember to change it once the input fitting energy range is different//
    float normX = 150.;
    float fluc = 1. * ( par[3] + par[4] * xx / normX + par[5] * (0.5*(3*xx*xx/normX/normX - 1.)) );
    return fluc;
}

__device__ float legendreSeries_lindhard(float xx, float *par ){
    //ATTENTION:normX is energymax, please remember to change it once the input fitting energy range is different//
    float normX = 150.;
    float fluc = 1. * ( par[6] + par[7] * xx / normX  + par[8] * (0.5*(3*xx*xx/normX/normX - 1.)));
    return fluc;
}
...
__device__ void get_yield_pars(bool simuTypeNR, float E_drift, float energy, float density, float * pars, float * free_pars){
    ...
    if(simuTypeNR){
        ...
        pars[0] = Wq_eV;
        pars[1] = L + legendreSeries_lindhard(energy, &free_pars[0]);
        pars[2] = NexONi;
        pars[3] = rmean + legendreSeries_rmean(energy, &free_pars[0]);
        pars[4] = omega;
        return;
    }
    else{
...
__device__ float get_chargeloss(float * chargeloss_array, float x, curandState_t *rand_state){
    ...
    float dice = ymin + (ymax - ymin) * curand_uniform(rand_state);
    float y = interpolate1d_cl(dice, &chargeloss_array[index_begin]);
    return y;
}

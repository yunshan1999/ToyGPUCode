...
inds = np.where((-7400<=lnls[:,-1]))[0]
#inds = np.where((-7476.757684<=lnls[:,-1])&(-2000>=lnls[:,-1]))[0]
#inds = np.where((lnls[:,-1]>=-7000)&(acceptances>0.17))[0]
samples_plot = samples_orig[inds,nconv:,:].reshape((-1,ndim))

fig = corner.corner(samples_plot, labels=["$g1$", "$g2b$", "$segb$","flatER", "$scale$ $of$ $deltaR$", "$p0$","$p1$", "$p2$"],

...
DATA_PATH_dd = '../data/reduced_ana_DD_5KV_merged_fomcut.root'
DATA_PATH_ambe = '../data/reduced_ana_AmBe_5KV_merged_fomcut.root'
data_tree_dd = uproot.open(DATA_PATH_dd)["out_tree"]
data_tree_ambe = uproot.open(DATA_PATH_ambe)["out_tree"]

cS1_dd = data_tree_dd["qS1C_max"].array()
cS2b_dd = data_tree_dd["qS2BC_max"].array()
cS2_dd = data_tree_dd["qS2C_max"].array()

cS1_ambe = data_tree_ambe["qS1C_max"].array()
cS2b_ambe = data_tree_ambe["qS2BC_max"].array()
cS2_ambe = data_tree_ambe["qS2C_max"].array()

chargeloss_array = np.loadtxt("../data/chargeloss_cpdf_beta.dat", dtype=np.float32)
bls_array = np.loadtxt("../data/bls_cpdf.dat", dtype=np.float32)

from gpu_modules.TestSignalSimCuda_dd import pandax4t_signal_sim_dd
from gpu_modules.TestSignalSimCuda_ambe import pandax4t_signal_sim_ambe

start = drv.Event()
end = drv.Event()

GPUFunc_dd     = SourceModule(pandax4t_signal_sim_dd, no_extern_c=True).get_function("signal_simulation")
GPUFunc_dd_er  = SourceModule(pandax4t_signal_sim_dd, no_extern_c=True).get_function("signal_simulation_er")
GPUFunc_ambe     = SourceModule(pandax4t_signal_sim_ambe, no_extern_c=True).get_function("signal_simulation")
GPUFunc_ambe_er  = SourceModule(pandax4t_signal_sim_ambe, no_extern_c=True).get_function("signal_simulation_er")
...
def get_2dband(theta, binning):
    ...
    output_array_dd        = np.zeros(11+xbin*ybin*2, dtype=np.double)
    output_array_dd_er     = np.zeros(11+xbin*ybin*2, dtype=np.double)
    output_array_ambe        = np.zeros(11+xbin*ybin*2, dtype=np.double)
    output_array_ambe_er     = np.zeros(11+xbin*ybin*2, dtype=np.double)
    ...
    tArgs_ambe_er               = [
        ...
        drv.In(bls_array),
        drv.In(chargeloss_array),
        drv.InOut(output_array_ambe_er)
    ]

    GPUFunc_dd(*tArgs_dd, **d_gpu_scale)
    GPUFunc_dd_er(*tArgs_dd_er, **d_gpu_scale)
    GPUFunc_ambe(*tArgs_ambe, **d_gpu_scale)
    GPUFunc_ambe_er(*tArgs_ambe_er, **d_gpu_scale)

    #gpuContext.pop() #finish
    return output_array_dd, output_array_dd_er, output_array_ambe, output_array_ambe_er
...
def get_lnlikelihood(theta):
    ...
    hmc_dd, hmc_dd_er, hmc_ambe, hmc_ambe_er = get_2dband(theta, binning)
    ...
    hmc_total_dd = np.asarray(hmc_dd) + np.asarray(hmc_dd_er) + 1e-35 # the small value of 1e-35 is for avoiding calculation infinity
    hmc_total_ambe = np.asarray(hmc_ambe) + np.asarray(hmc_ambe_er) + 1e-35

    Nmc_dd = hmc_total_dd[1]
    Nmc_ambe = hmc_total_ambe[1]
    ...
    if Nmc_dd<1e6 or Nmc_ambe<1e6:    
        return -np.inf
    else: 
        global cS1_dd, cS2b_dd, cS2_dd
        global cS1_ambe, cS2b_ambe, cS2_ambe
        
        #be sure the data inside hist#
        
        inds_dd = np.where((cS1_dd>=binning[0][1])&(cS1_dd<binning[0][2])&(cS2_dd<8000)&(np.log10(cS2b_dd)>=binning[1][1])&(np.log10(cS2b_dd)<binning[1][2]))[0]
        inds_ambe = np.where((cS1_ambe>=binning[0][1])&(cS1_ambe<binning[0][2])&(cS2_ambe<8000)&(np.log10(cS2b_ambe)>=binning[1][1])&(np.log10(cS2b_ambe)<binning[1][2]))[0]
        cS1_dd_sel = cS1_dd[inds_dd]
        cS2b_dd_sel = cS2b_dd[inds_dd]
        cS1_ambe_sel = cS1_ambe[inds_ambe]
        cS2b_ambe_sel = cS2b_ambe[inds_ambe]

        bin_inds_dd = get_bin_num(cS1_dd_sel,np.log10(cS2b_dd_sel),binning)
        bin_inds_ambe = get_bin_num(cS1_ambe_sel,np.log10(cS2b_ambe_sel),binning)
        
        #print(bin_inds_dd.shape, bin_inds_ambe.shape)
        lnLikelihood_total=np.sum(np.log(hmc_total_dd[bin_inds_dd]/Nmc_dd)) + np.sum(np.log(hmc_total_ambe[bin_inds_ambe]/Nmc_ambe))
...
def get_lnprior(theta):
    ...
    if 0.< g1 <0.2 and g2b > 0. and segb > 0. and theta[3] > 0. and theta[7] > 0. and theta[8] > 0.:
...
#p0 = [[nuisance_mean[0], nuisance_mean[1], nuisance_mean[2], 1., 0.01, 0.01, 0.01, 0.17, 0.04]*(1 + 0.1*np.random.randn(ndim)) for i in range(nwalkers)]
...
axs[2, 1].set_title('tritium ER(DD)',fontsize=7)

axs[2, 2].plot(samples[:,:,8].T, linewidth = 0.1, color = "black")
axs[2, 2].set_title('tritium ER(AmBe)',fontsize=7)
